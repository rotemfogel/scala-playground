...
object GlueSync {
  ...
  private final val LATEST: String = "_latest"
  final val MARIADB: String = "mariadb"
  final val TEST_MARIADB: String = "test_mariadb"
  final val DBL: String = "dbl"
  final val TEST_DBL: String = "test_dbl"
  final val DBR: String = "dbr"
  private final val TEST_DBR: String = "test_dbr"
  final val ZUORA: String = "zuora"
  final val TEST_ZUORA: String = "test_zuora"
  private final val CREATE_VIEW: String = s"CREATE OR REPLACE VIEW %s AS SELECT global_hash_key,commit,database,ts,type,xid,xoffset,data,old,tbl,date_,hour FROM (SELECT row_number() OVER (PARTITION BY tbl,global_hash_key ORDER BY ts DESC,is_deleted DESC,is_updated DESC,is_inserted DESC) row_num,* FROM (SELECT (CASE WHEN (type = 'delete') THEN true ELSE false END) is_deleted,(CASE WHEN (type = 'insert') THEN true ELSE false END) is_inserted,(CASE WHEN (type = 'update') THEN true ELSE false END) is_updated,* FROM (SELECT * FROM testfromDBl.maria_dim_incr_latest WHERE (tbl = '%s') UNION ALL SELECT * FROM testfromDBl.maria_dim_latest WHERE (tbl = '%s')) a) b) c WHERE ((row_num = 1) AND (is_deleted = false))"
  ...
  private final val TEST: String = "test_"

  def filterLatest(name: String): Boolean = name.endsWith(LATEST)

  def filterNotLatest(name: String): Boolean = !name.endsWith(LATEST)

  private def defaultFilter(name: String): Boolean = !name.equals("table_name")

  private final val athena: AmazonAthena = AmazonAthenaClientBuilder.standard().withRegion(Regions.US_WEST_2).build()

  def main(args: Array[String]): Unit = {
    createTables(TEST_DBR, fn = (s: String) => s.equals("paying_subscriptions"), force = true)
    // createTables(DBL, (s: String) => s.startsWith("maria") && filterNotLatest(s))
    // createViews(DBL, (s: String) => s.startsWith("maria") && filterLatest(s))
    // createTables(ZUORA)
  ...
  def createTables(fromDB: String, toDB: Option[String] = None, fn: String => Boolean = defaultFilter, force: Boolean = false): Unit = {
    val glue: AWSGlue = AWSGlueClientBuilder.standard().withRegion(Regions.US_WEST_2).build()

    def getPartitions(db: String, table: String): Seq[Partition] = {
      var partitions = Seq[Partition]()

      def fetchPartitions(nextToken: Option[String] = None): Option[String] = {
        var partitionRequest = new GetPartitionsRequest().withDatabaseName(db).withTableName(table)
        if (nextToken.isDefined) partitionRequest = partitionRequest.withNextToken(nextToken.get)
        val result = glue.getPartitions(partitionRequest)
        partitions ++= result.getPartitions.asScala
        val token = {
          val t = result.getNextToken
          t match {
            case null => None
            case _ => Some(t)
          }
      ...
      // loop until partitions is full
      var token: Option[String] = fetchPartitions()
      while (token.isDefined) {
        token = fetchPartitions(token)
      }
      partitions
    }

    logger.info("obtaining list of tables...")
    val dbFile = TABLES_FILE.format(fromDB)
    ...
    val tables = {
      ...
      if (lines.nonEmpty) {
        ...
        val queryExecutionId: String = execute(fromDB, GET_TABLES.format(fromDB))
    ...
    val db: String = toDB match {
      case Some(x) => x
      case None => if (fromDB.startsWith(TEST)) fromDB.substring(TEST.length) else s"$TEST$fromDB"
    }
    tables.foreach(table => {
      try {
        val getTableResult: GetTableResult = glue.getTable(new GetTableRequest().withDatabaseName(fromDB).withName(table))
        if (getTableResult.getTable != null) {
          val sourceTable: Table = getTableResult.getTable
          logger.info(s"got table ${sourceTable.getDatabaseName}.${sourceTable.getName}")
          val sourcePartitions = getPartitions(fromDB, table)
          logger.info(s"got ${sourcePartitions.size} partitions ${sourceTable.getDatabaseName}.${sourceTable.getName}")
          val targetTable: Option[Table] =
            try {
              val target = glue.getTable(new GetTableRequest().withDatabaseName(db).withName(table)).getTable
              logger.info(s"got table ${target.getDatabaseName}.${target.getName}")
              val targetPartitions = getPartitions(db, table)
              logger.info(s"got ${targetPartitions.size} partitions ${target.getDatabaseName}.${target.getName}")
              if (force || sourcePartitions.size != targetPartitions.size && targetPartitions.nonEmpty) {
          ...
          val location = sourceTable.getStorageDescriptor.getLocation.replaceAll(fromDB, db).replaceAll(BUCKET, s"$BUCKET-dev")
          ...
          if (sourcePartitions.nonEmpty) {
            val groups: Iterator[Seq[Partition]] = sourcePartitions.grouped(100)
            groups.foreach(group => {
              val list = group.map(p => new PartitionInput()
                .withParameters(p.getParameters)
                .withLastAccessTime(p.getLastAccessTime)
                .withLastAnalyzedTime(p.getLastAnalyzedTime)
                .withStorageDescriptor(p.getStorageDescriptor)
                .withValues(p.getValues)).asJava
              glue.batchCreatePartition(new BatchCreatePartitionRequest().withDatabaseName(db).withTableName(table).withPartitionInputList(list))
            })
          }
        }
  ...
  def createViews(fromDB: String, toDB: Option[String] = None, fn: String => Boolean = defaultFilter): Unit = {
    logger.info("obtaining list of views...")
    val dbFile = VIEWS_FILE.format(fromDB)
    val start = System.currentTimeMillis()
    val views = {
      var lines: List[String] = List()
      try {
